...
<form method="post" id="code-form">
    ...
    <select name="mode" id="mode">
        ...
        <option value="java" {% if mode=='java' %}selected{% endif %}>Java</option>
    </select>
    ...
    if request.method == "POST":
        ...
        if mode == "python":
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".java") as tmp:
                ...
                comp = subprocess.run(["javac", src_path], capture_output=True)

                if comp.returncode != 0:
                    output = comp.stderr.decode()
                else:
                    run = subprocess.run(["java", "-cp", dir_path, base], capture_output=True)
                    output = run.stdout.decode() + run.stderr.decode()

                os.unlink(src_path)
                class_file = os.path.join(dir_path, base + ".class")
